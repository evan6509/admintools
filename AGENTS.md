...
Viewers are additionally toggleable via config (`enable_inventory_viewer`, etc.).
...
- Permission gates use UUID-keyed grants from `PermissionManager`, with OP4 as a
  universal bypass.
...
- Make yourself OP4 in the server console (`op <name>`) to manage AdminTools access.
