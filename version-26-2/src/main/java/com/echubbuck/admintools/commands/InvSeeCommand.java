...
public class InvSeeCommand {
    ...
    private static int execute(ServerCommandSource source, ServerPlayerEntity target) {
        ...
        if (!AdminToolsMod.getConfigManager().getBoolean("enable_inventory_viewer", true)) {
            source.sendError(Text.literal("Inventory viewer is disabled."));
            return 0;
        }

        admin.openHandledScreen(new InvSeeScreenHandler.Provider(target));
        AdminToolsMod.getActionLogger().log(source.getName(), "INVSEE", target.getName().getString(), "opened");
        return 1;
    }
}
